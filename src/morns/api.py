...
MAP_WINDOWS = {5, 30, 60, 360, 1440, 2880, 10080, 20160, 43200, 129600, 259200, 525600}
...
def create_app(settings: Settings | None = None, store: ObservationStore | None = None) -> FastAPI:
    ...
    def receiver_config() -> dict[str, Any]:
        ...
        return {
            ...
            "map_windows_minutes": setup.get("map_windows_minutes", sorted(MAP_WINDOWS)),
    ...
    def setup_screen() -> str:
        return files("morns").joinpath("templates/setup.html").read_text(encoding="utf-8")
    ...
    def stats() -> dict:
        ...
        store.enforce_retention(
            receiver["observation_retention_days"], receiver["message_retention_days"]
        )
        return {
            "station": receiver["station_name"],
            "observation_retention_days": receiver["observation_retention_days"],
    ...
    def base_station_stats(minutes: int = Query(60)) -> dict[str, Any]:
        if minutes not in ALLOWED_WINDOWS:
            minutes = 60
        measured = store.base_station_stats(minutes)
        last = measured.get("last_received_at")
        age_seconds = None
        if last:
        ...
        elif age_seconds is not None and age_seconds > 600:
            health_state, health_reason = "stale", "No observation has arrived in the last 10 minutes"
        else:
            health_state, health_reason = "healthy", "Observations are arriving"
        return {
            "station": receiver_config()["station_name"],
